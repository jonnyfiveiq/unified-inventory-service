...
API (pyVmomi) and collects the full inventory hierarchy:
    ...
    Datacenters -> Clusters -> Hosts -> VMs
                             -> Resource Pools
                  -> Datastores
                  -> Networks (dvSwitches, port groups)
...
Resource type mapping (vSphere -> normalized taxonomy):
    ...
    DistributedVirtualSwitch -> virtual_switch
    Network / dvPortGroup    -> virtual_network

Requirements:
    pip install pyvmomi

Connection parameters (via Provider.connection_config):
...
from apps.inventory.providers.base import BaseProvider, ProviderCredential, ResourceData

logger = logging.getLogger("apps.inventory.providers.vmware_vsphere")

# vSphere managed object type -> normalized resource_type slug
VSPHERE_TYPE_MAP = {
    "VirtualMachine": "virtual_machine",
    "HostSystem": "hypervisor_host",
    "ClusterComputeResource": "container_orchestration_platform",
...
def _first_id_value(id_info_list, key: str) -> str:
    """Extract a value from vSphere's otherIdentifyingInfo list."""
    if not id_info_list:
        return ""
    for info in id_info_list:
        try:
            if hasattr(info, "identifierType") and info.identifierType:
                if getattr(info.identifierType, "key", "") == key:
...
def _normalize_os_type(guest_id: str) -> str:
    """Map vSphere guestId to a normalized OS type."""
    if not guest_id:
        return "other"
    gid = guest_id.lower()
    if any(w in gid for w in ("windows", "win31", "win95", "win98", "winnt")):
        return "windows"
    if any(w in gid for w in ("rhel", "centos", "ubuntu", "debian", "suse",
...
def _suggest_ansible_connection(os_type: str) -> str:
    """Suggest an Ansible connection type based on OS."""
    if os_type == "windows":
        return "winrm"
    if os_type in ("linux", "bsd", "macos"):
        return "ssh"
    return ""


def _suggest_inventory_group(os_type: str, is_template: bool, tags: dict) -> str:
    """Suggest an Ansible inventory group based on OS and metadata."""
    if is_template:
        return ""
    if os_type == "windows":
        return "windows"
...
class VMwareVSphereProvider(BaseProvider):
    ...
    def collect(self) -> Iterator[ResourceData]:
        ...
        Yields resources in dependency order:
        1. Clusters (so hosts can reference them)
        2. Hosts
        3. Resource Pools
        4. Datastores
        5. VMs (reference hosts, datastores, pools)
        """
        from pyVmomi import vim
    ...
    def _collect_hosts(self, content, dc, dc_name: str) -> Iterator[ResourceData]:
        ...
        for host in container.view:
            ...
            # Extract SMBIOS / BIOS UUID from hardware
            bios_uuid = ""
            serial_number = ""
            hw_model = ""
            hw_vendor = ""
            ...
            ip = ""
            if config_summary:
                mgmt_ip = getattr(config_summary, "name", "") or ""
                ip = mgmt_ip

            # Connection state
            runtime = host.runtime
            connection_state = getattr(runtime, "connectionState", "unknown") or "unknown"
            power_state_raw = getattr(runtime, "powerState", "") or ""
            in_maintenance = getattr(runtime, "inMaintenanceMode", False)

            state = "active"
            if connection_state == "disconnected":
                state = "disconnected"
            elif connection_state == "notResponding":
                state = "error"
            elif in_maintenance:
                state = "maintenance"
            ...
            yield ResourceData(
                ...
                name=host.name,
                canonical_id=bios_uuid or f"vsphere:host:{moid}",
                vendor_identifiers={
                    "moid": moid,
                    "bios_uuid": bios_uuid,
                ...
                properties={
                    ...
                    "esxi_version": getattr(config_summary, "product", None)
                        and getattr(config_summary.product, "version", "") or "",
                    "esxi_build": getattr(config_summary, "product", None)
                        and getattr(config_summary.product, "build", "") or "",
    ...
    def _collect_resource_pools(self, content, dc, dc_name: str) -> Iterator[ResourceData]:
        ...
        for pool in container.view:
            moid = pool._moId
            # Skip the implicit "Resources" root pool of each cluster
        ...
        for ds in container.view:
            ...
            yield ResourceData(
                ...
                properties={
                    ...
                    "_metrics": {"provisioned_pct": provisioned_pct},
                },
            )
        container.Destroy()
    ...
    def _collect_vms(self, content, dc, dc_name: str) -> Iterator[ResourceData]:
        ...
        for vm in container.view:
            ...
            vm_name = vm.name
            is_template = getattr(config, "template", False) if config else False
            ...
            # Power state
            power_state_raw = getattr(runtime, "powerState", "") or ""
            state = POWER_STATE_MAP.get(power_state_raw, "unknown")
            if is_template:
                state = "template"
            ...
            if hasattr(runtime, "bootTime") and runtime.bootTime:
                bt = runtime.bootTime
                if hasattr(bt, "replace"):
                    boot_time = bt.replace(tzinfo=dt_timezone.utc) if bt.tzinfo is None else bt
            ...
            if config and config.hardware:
                ...
                # Sum virtual disk capacities
                for device in config.hardware.device:
                    if isinstance(device, vim.vm.device.VirtualDisk):
                        cap = getattr(device, "capacityInKB", 0) or 0
                        disk_gb += cap // (1024 * 1024)
            ...
            if guest:
                ...
                ip_addr = getattr(guest, "ipAddress", "") or ""
                if ip_addr:
                    ip_addresses.append(ip_addr)
                # Collect all IPs and MACs from guest NICs
                if hasattr(guest, "net") and guest.net:
                    for nic in guest.net:
                        mac = getattr(nic, "macAddress", "") or ""
                        if mac and mac not in mac_addresses:
                            mac_addresses.append(mac)
                        if hasattr(nic, "ipConfig") and nic.ipConfig:
                            for ip_info in (nic.ipConfig.ipAddress or []):
                                addr = getattr(ip_info, "ipAddress", "") or ""
                                if addr and addr not in ip_addresses:
                                    ip_addresses.append(addr)

            # OS info
            guest_id = getattr(config, "guestId", "") or "" if config else ""
            guest_full_name = getattr(config, "guestFullName", "") or "" if config else ""
            os_type = _normalize_os_type(guest_id)
            os_name = guest_full_name

            # Host, cluster, resource pool, datastore relationships
            host_moid = ""
            host_name = ""
            cluster_name = ""
            pool_name = ""
            ...
            if runtime and hasattr(runtime, "host") and runtime.host:
            ...
            # Linked clone detection
            linked_clone = _is_linked_clone(config)

            # Ansible connection suggestion
            ansible_connection = _suggest_ansible_connection(os_type)
            ...
            if host_moid:
                relationships.append({
                    "target_ems_ref": host_moid,
                    "relationship_type": "runs_on",
                })
            if pool_moid and pool_name != "Resources":
                relationships.append({
                    "target_ems_ref": pool_moid,
                    "relationship_type": "member_of",
                })
            if datastore_moid:
                relationships.append({
                    "target_ems_ref": datastore_moid,
                    "relationship_type": "attached_to",
                })

            # Quick stats / metrics
            metrics = {}
            qs = summary.quickStats if summary else None
            if qs:
                cpu_usage = getattr(qs, "overallCpuUsage", 0) or 0
                if cpu_count > 0 and cpu_usage > 0:
                    metrics["cpu_usage_pct"] = round(cpu_usage / (cpu_count * 1000) * 100, 1)
